...
- **Production = `main` = `launch-prep`**, deployed behind the maintenance gate (all 3 launch flags still ON). Latest deploys include: mobile card glow + a11y contrast fix, Facebook/Meta domain-verification tag, and the **custom-shirt order-flow overhaul** (flat ₪149 · Lycra/Look added · Stone-wash shown · per-model colours · all prints included · Oversize category · "Our Fabrics" guide — see 2026-06-05 below). Public still sees the maintenance page; launch = flip the 3 flags.
...
- `PAYMENTS_ENABLED=true`. `create-payment` is at **v11**, `tranzila-webhook` at **v12** (`VERIFY_MODE="enforce"`, real query-back to `report.tranzila.com`). See *Tranzila integration* below for the full payment-integrity model.
...
When going public: **flip the 3 launch flags.** (Note: `launch-prep` now has 1 unmerged polish commit `9e7dc2f` — merge it to `main` first if you want the mobile card glow + a11y contrast fix live.)
