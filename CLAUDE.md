...
- **Production = `main` = `25406b5`**, deployed behind the maintenance gate (all 3 launch flags still ON). **`launch-prep` is now 1 commit AHEAD at `9e7dc2f`** (mobile card glow + a11y contrast-overlay fix — see 2026-06-05 below); **NOT yet merged to `main`, NOT deployed.** To ship that polish later: merge `launch-prep`→`main` (which deploys) — separate from the launch-flag flip.
