...
- **`launch-prep` is 2 commits AHEAD of `main`** (HEAD `2798059`, **not deployed to main**): `d92eeea` create-payment quantity clamp (security, **edge fn NOT deployed** — live is still v19) · `2798059` reviews localization (per-language author name + product). Pushed to origin. Merge to `main` when ready.
...
- `PAYMENTS_ENABLED=true`. **Live edge-fn versions: `create-payment` v19** (prices commission by type×product, server-side), **`tranzila-webhook` v18** (`VERIFY_MODE="enforce"`, real query-back to `report.tranzila.com`). See *Tranzila integration* below for the full payment-integrity model. ⚠️ Known low-risk hole (deferred by owner 2026-06-06): commission `extra_prints.ctype`/`pid` are client-supplied and used directly server-side → a crafted insert could pay the cheaper tier; fix = validate ctype/pid against an allowlist + cross-check `row.product` in `create-payment` (needs a prod deploy).
