...
When going public: **flip the 3 launch flags.** (`main` == `launch-prep` == `6e29579` — fully in sync, nothing unmerged.)
...
- **Keep unchanged:** `PAYMENTS_ENABLED=true`, `STONEWASH_ENABLED=false`, `MUG_STUDIO_ENABLED=false`. `SHIPPING_PRICE=35` kept (internal fallback only — see Delivery model). `WHATSAPP_NUMBER` live ✓. **Launch = flip the 3 flags.** (`main` == `launch-prep`, fully in sync.)
