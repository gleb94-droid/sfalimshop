                        ...
                        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                          ...
                          {order.design_url && (
                            <div>
                              ...
                              <a href={order.design_url} download target="_blank" rel="noreferrer" style={{ display: "inline-block", background: "rgba(255,107,53,0.15)", border: "1px solid #FF6B35", color: "#FF6B35", borderRadius: 6, padding: "6px 12px", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>⬇️ Download</a>
                            </div>
                          )}
                          <div>
  ...
  const handleSubmit = async () => {
    ...
    if (uploadedImage) {
      try {
        ...
        const { data: uploadData } = await supabase.storage.from('designs').upload(fileName, blob);
        if (uploadData) {
